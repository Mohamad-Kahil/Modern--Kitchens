      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          {[1, 2, 3, 4].map((item) => (
            <div
              key={item}
              className="bg-card p-4 rounded-lg border border-border shadow-light-card dark:bg-micro-dark light:bg-white"
            >
        ...
        {submodules.map((submodule) => (
          <TabsContent
            key={submodule.id}
            value={submodule.id}
            className="border rounded-lg p-6 bg-card min-h-[400px] dark:bg-micro-dark light:bg-white shadow-light-card h-[calc(100vh-320px)] relative overflow-hidden"
          >
            {submodule.content ? (
              submodule.content
            ) : (
